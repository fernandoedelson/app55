...
DESENHO.carteira=function(P){
  ...
  const aging=(ag,total)=>fig(vbars(ag.map(x=>[x[0],x[1]]),{valfmt:v=>mi(v,1),w:820,h:280,colors:cols}),ins('auto'))
    +table(['Faixa','Valor pendente','Pedidos'],ag.map(x=>[esc(x[0]),money(x[1]),nf(x[2])]),['left','right','right'],
      ['Total',money(total),nf(ag.reduce((a,x)=>a+x[2],0))],ins('auto'));
  if(A){
  ...
  if(S){ const stat=S.stat, stTot=stat.reduce((a,x)=>a+x[1],0);
    ...
    s+=fig(pareto(stat,{valfmt:v=>mi(v,1),colors:[SER[0],GOOD,WARN,BAD]}),ins('auto'));
  ...
  if(V||D){
    s+='<div class="two">';
    if(V) s+='<div>'+H3('Carteira por vendedor(a)','','ce-vend')+table(['Vendedor(a)','Pendente','Pedidos'],V.vend.map(x=>[esc(x[0]),money(x[1]),nf(x[2])]),['left','right','right'],null,ins('auto'))+'</div>';
    if(D) s+='<div>'+H3('Maiores pedidos em carteira','','ce-peds')+table(['Pedido','Cliente','Pendente'],D.peds.map(x=>['FOCCO-'+esc(x[0]),esc(trunc(x[1],22)),money(x[2])]),['left','left','right'],null,ins('auto'))+'</div>';
    s+='</div>';
  }
  return s;
};
