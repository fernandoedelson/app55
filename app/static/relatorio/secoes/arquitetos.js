...
function _aqCorpo(P){
  ...
  if(D){
    ...
    s+=fig(stackedCols(D.nomes.map(n=>trunc(n,14)),[
      ...
      ],{valfmt:v=>mi(v,1),w:960,subLabels:D.share.map(v=>pct(v,0)),subTitle:'% via arquiteto'}),ins('arqCanal'));
  ...
  if(M){ const md={}; M.linhas.forEach((vn,i)=>{md[vn]={}; M.colunas.forEach((an,j)=>md[vn][an]=M.mat[i][j]);});
    ...
    s+=fig(heatmap(M.linhas,M.colunas,md,{valfmt:v=>v>1000?nf(v/1000,0):'',padLeft:150,w:940}),ins('arqMatriz'));
  ...
  if(L){
    ...
    s+=fig(hbar(L.top,{valfmt:v=>mi(v,1),padLeft:250,w:820,color:SER[6],maxbars:14,pct:true,total:L.total}),ins('auto'));
    s+=table(['Arquiteto / escritório','Venda atribuída','RT paga','RT efetiva','Pedidos','Clientes',
      ...
      ['left','right','right','right','right','right','left','right','right'],null,ins('arqExclusividade'));
  ...
  if(PA){
    ...
    s+=table(['Arquiteto / escritório','Venda na janela','Pedidos','Meses sem trazer','Vendedor principal'],
      ...
      ['left','right','right','right','left'],null,ins('arqParados'));
  }
  const R=P['aq-rfv'];
  if(R){ const segCores=AQ_SEG_CORES();
    ...
    s+=fig(hbar(R.segs.map(x=>[x[0],x[3]]),{valfmt:v=>mi(v,1),padLeft:170,w:820,color:SER[6]}),ins('rfvArqSegmentos'));
  ...
  if(PF){
    ...
    s+=fig(stackedCols(PF.nomes.map(n=>trunc(n,14)),[
      ...
      ],{valfmt:v=>mi(v,1),w:960,subLabels:PF.sub.map(v=>pct(v,0)),subTitle:'% em fiéis'}),ins('rfvArqPorVendedor'));
    s+=table(['Vendedor(a)','Arquitetos','Venda pelo canal','Campeões + fiéis','% do valor dele',
      ...
      PF.linhas.map(x=>[esc(trunc(x[0],22)),nf(x[1]),money(x[2]),
        ...
        nf(x[7],2),money(x[8])]),
      ['left','right','right','right','right','right','right','right','right'],null,ins('rfvArqQualidade'));
