/* ===== +55 Design · Roteiro em Atos =====================================
   ...
   ---- versao da aplicacao +55 (app/apresentacao/atos_app.js) ----
   Copia do atos.js do Kit congelado com quatro acrescimos, todos marcados "[app]":
   o roteiro vem de window.ROTEIRO_55 (pilotado na aplicacao; sem ele vale o do Kit),
   o anexo esconde o que ja esta num ato (e os quadros equivalentes), o menu nao lista
   secao vazia, e o ato marcado com cortina abre com a cortina espresso do Sumario.
   Os comentarios aprovados para a reuniao aparecem sob o titulo do grafico.
   ====================================================================== */
'use strict';
(function(){
...
var R55=window.ROTEIRO_55||null;   /* [app] o roteiro pilotado na aplicacao */
var ATOS=(R55&&R55.atos)||[
...
/* [app] o anexo so mostra o que nao esta nos atos: some o quadro que um ato ja traz
   (ou o equivalente dele em outra secao). Esconde, nao remove — os filtros redesenham. */
var OCULTAR={};
((R55&&R55.ocultar)||[]).forEach(function(b){ OCULTAR[b]=1; });
function esconderRepetidos(){
  if(!R55) return;
  document.querySelectorAll('main > section:not(.ato) [data-blk-of]').forEach(function(el){
...
function esconderVazias(){
  ...
  var nav=$('nav');
  if(nav) nav.querySelectorAll('.nav-grp').forEach(function(h){
    var algum=false, el=h.nextElementSibling;
    while(el && !el.classList.contains('nav-grp')){
      if(el.tagName==='A' && !el.classList.contains('vazio')) algum=true;
      el=el.nextElementSibling;
    }
    h.classList.toggle('vazio',!algum);
  });
  mostrarComentarios();
}

/* [app] comentarios que a Controladoria aprovou para a reuniao, sob o titulo do grafico */
function mostrarComentarios(){
  var C=window.CMT_REUNIAO; if(!C) return;
  Object.keys(C).forEach(function(blk){
    var h=document.querySelector('h3[data-blk="'+blk+'"]');
    if(!h || (h.nextElementSibling && h.nextElementSibling.classList.contains('r55-cmt'))) return;
    var d=document.createElement('div'); d.className='r55-cmt';
    d.innerHTML=C[blk].map(function(c){
      return '<p><span class="r55-cmt-a">'+esc(c.area)+'</span>'+esc(c.texto)+'</p>'; }).join('');
...
/* [app] o ato com cortina abre como o Sumario: a cortina espresso desce, a pagina vai
   ao ato por tras dela e a cortina sobe. O #curtain e o do app.js (setupCurtain); sem
   ele — movimento reduzido — o link navega direto, como qualquer outro. */
function ligarCortinas(){
  ATOS.forEach(function(A){
    if(!A.cortina) return;
    var a=document.querySelector('#nav a[data-t="ato'+A.n+'"]'); if(!a) return;
    a.addEventListener('click',function(e){
      var alvo=$('ato'+A.n); if(!alvo || !$('curtain')) return;
      e.preventDefault();
      document.body.classList.add('curtain-on');
      setTimeout(function(){
        window.scrollTo({top:topoAbs(alvo),behavior:'instant'});
        history.replaceState(null,'','#ato'+A.n);
        setTimeout(function(){ document.body.classList.remove('curtain-on'); },80);
      },430);
    });
  });
}
...
function ajustarFerramentas(){
  ...
  if(rod){
    var NUM=['Nenhum ato','Um ato','Dois atos','Três atos','Quatro atos','Cinco atos','Seis atos','Sete atos',
             'Oito atos','Nove atos','Dez atos','Onze atos','Doze atos'];
    var min=ATOS.reduce(function(t,A){ var m=parseInt(A.min,10); return t+(isNaN(m)?0:m); },0);
    rod.textContent=(NUM[ATOS.length]||(ATOS.length+' atos'))+(min?' · '+min+' min':'')+' · anexos completos ao fim.';
  }
}


/* [app] estilos dos acrescimos da aplicacao */
(function(){
  var st=document.createElement('style');
  st.textContent='.r55-fora{display:none!important}'
    +'#nav a.vazio,#nav .nav-grp.vazio{display:none!important}'
    +'.r55-cmt{margin:-4px 0 16px;padding:10px 14px;border-left:3px solid #577c69;background:rgba(87,124,105,.07);'
    +'font:400 14px/1.55 var(--sans);color:var(--ink)}'
    +'.r55-cmt p{margin:0}.r55-cmt p+p{margin-top:6px}'
    +'.r55-cmt-a{display:inline-block;margin-right:8px;font-size:11px;font-weight:600;letter-spacing:.12em;'
    +'text-transform:uppercase;color:#577c69}';
